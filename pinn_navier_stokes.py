...
def tgv_p(t: np.ndarray, x: np.ndarray, y: np.ndarray, nu: float) -> np.ndarray:
    return (-(np.cos(2.0 * x) + np.cos(2.0 * y)) * 0.25 * np.exp(-4.0 * nu * t)).astype(np.float32)
...
class MLP(nn.Module):
    ...
    def __init__(self, in_dim: int = 3, hidden_layers: int = 8, width: int = 64, out_dim: int = 3) -> None:
        super().__init__()
    ...
    def forward(self, txy: Tensor) -> Tensor:
        """
        Parameters
        ----------
        txy : Tensor, shape (N, 3)
            Concatenated inputs [t, x, y].
...
class NavierStokesPINN:
    ...
    def __init__(self, model: nn.Module, cfg: NSConfig, device: str | torch.device = "cpu") -> None:
    ...
    def _autograd_grads(self, u: Tensor, vars: Tuple[Tensor, ...]) -> Tuple[Tensor, ...]:
        """
        Compute first derivatives of u wrt each variable in `vars`.
        """
        ones = torch.ones_like(u)
        return tuple(torch.autograd.grad(u, v, ones, retain_graph=True, create_graph=True)[0] for v in vars)

    def pde_residuals(self, t: Tensor, x: Tensor, y: Tensor) -> Tuple[Tensor, Tensor, Tensor]:
        """
        ...
        p_t, p_x, p_y = self._autograd_grads(p, (t, x, y))  # p_t unused, but computed for completeness
    ...
    def _make_training_points(self, tcfg: TrainConfig) -> Dict[str, Tensor]:
        ...
        # IC: t = 0, random (x,y)
        x0 = np.random.uniform(self.cfg.xmin, self.cfg.xmax, size=(tcfg.n_ic, 1)).astype(np.float32)
        y0 = np.random.uniform(self.cfg.ymin, self.cfg.ymax, size=(tcfg.n_ic, 1)).astype(np.float32)
        t0 = np.zeros_like(x0, dtype=np.float32)
        u0 = tgv_u(t0, x0, y0, self.cfg.nu)
        v0 = tgv_v(t0, x0, y0, self.cfg.nu)
        p0 = tgv_p(t0, x0, y0, self.cfg.nu)  # pressure defined up to constant; consistent here

        # Interior collocation via LHS in unit cube -> map to domain
        H = latin_hypercube(tcfg.n_f, 3, 0.0, 1.0)
        tf = (self.cfg.tmin + (self.cfg.tmax - self.cfg.tmin) * H[:, [0]]).astype(np.float32)
        xf = (self.cfg.xmin + (self.cfg.xmax - self.cfg.xmin) * H[:, [1]]).astype(np.float32)
        yf = (self.cfg.ymin + (self.cfg.ymax - self.cfg.ymin) * H[:, [2]]).astype(np.float32)

        # Periodicity samples: pair (x=0,y) with (x=L,y), and (y=0) with (y=L)
        # Sample y for x-faces; sample x for y-faces
        y_per = np.random.uniform(self.cfg.ymin, self.cfg.ymax, size=(tcfg.n_per, 1)).astype(np.float32)
        x_per = np.random.uniform(self.cfg.xmin, self.cfg.xmax, size=(tcfg.n_per, 1)).astype(np.float32)
        t_per = np.random.uniform(self.cfg.tmin, self.cfg.tmax, size=(tcfg.n_per, 1)).astype(np.float32)
        ...
        return {
            # IC
            "t0": to_t(t0), "x0": to_t(x0), "y0": to_t(y0),
            "u0": to_t(u0), "v0": to_t(v0), "p0": to_t(p0),
            # Interior
            "tf": to_t(tf), "xf": to_t(xf), "yf": to_t(yf),
            # Periodic faces
            "tper": to_t(t_per), "yper": to_t(y_per), "xper": to_t(x_per),
    ...
    def train(
        ...
        def loss_fn() -> Tensor:
            ...
            lic = torch.mean((u0_pred - data["u0"])**2) + \
        ...
        for step in range(1, tcfg.adam_steps + 1):
            ...
            if step % 1000 == 0 or step == 1:
                print(f"[NS-PINN] step={step:6d}  loss={L.item():.6e}")
        ...
        if tcfg.lbfgs_max_iter > 0:
            ...
            def closure() -> Tensor:
                ...
                l.backward()
                return l
            print("[NS-PINN] L-BFGS ...")
            lb.step(closure)
            print("[NS-PINN] L-BFGS done.")

    # ---------- Inference ----------

    @torch.no_grad()
    def predict(self, t: np.ndarray, x: np.ndarray, y: np.ndarray, batch: int = 4096) -> np.ndarray:
        """
...
def demo_tgv() -> None:
    ...
    cfg = NSConfig(tmin=0.0, tmax=1.0, xmin=0.0, xmax=2.0*math.pi, ymin=0.0, ymax=2.0*math.pi, nu=0.01)
